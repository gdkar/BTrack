...
string
BTrackVamp::getIdentifier() const{return "btrack-vamp";}

string
BTrackVamp::getName() const{return "BTrack";}

string
BTrackVamp::getDescription() const{return "A Real-Time Beat Tracker";}
...
BTrackVamp::getInputDomain() const{return TimeDomain;}
size_t
...
size_t
BTrackVamp::getMinChannelCount() const{return 1;}
size_t
BTrackVamp::getMaxChannelCount() const{return 1;}
...
{
    if (identifier == "parameter") {return 5; }
    return 0;
}
...
string
BTrackVamp::getCurrentProgram() const{return ""; /* no programs */}
void
...
void
BTrackVamp::reset(){/* Clear buffers, reset stored values, etc*/}
...
BTrackVamp::process(const float *const *inputBuffers, Vamp::RealTime timestamp){
    ...
    float frame[m_blockSize];
    // copy samples into our frame
    for (int i = 0;i < m_blockSize;i++)
    {frame[i] = (float) inputBuffers[0][i];}
...
BTrackVamp::FeatureSet
BTrackVamp::getRemainingFeatures(){return FeatureSet();}
