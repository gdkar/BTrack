...
It make_kaiser_bessel_derived_window(It _beg, It _end, float alpha)
{
    ...
    std::transform(_beg,_mid,_beg,[_norm](auto x){return std::sqrt(x * _norm);});
...
It make_xiph_vorbis_window(It _beg, It _end)
{
    ...
    return std::transform(_beg,_end,_beg,[pi_over_N,half_pi](auto x) {
        return std::sin(half_pi * sqr(std::sin(x * pi_over_N)));
    });
}
