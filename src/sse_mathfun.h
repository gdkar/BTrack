...
static inline v4sf _approx_rsqrt_ps( v4sf x )
{
  ...
  const v4sf minus_half_x =
    _mm_mul_ps(*(const v4sf*)_ps_isqrt_c0 ,x );
  const v4sf y0_2 = _mm_mul_ps(y0,y0);
  const v4sf three_halfs_y0 =
    _mm_mul_ps(*(const v4sf*)_ps_isqrt_c1,y0);
...
static inline v4sf _approx_atan2_ps ( v4sf y, v4sf x )
{
  ...
  v4sf angle2= _mm_mul_ps(angle,angle);
       ...
       angle2= _mm_sub_ps(angle2, *(const v4sf*)_ps_cephes_PIO4);
       angle2= _mm_xor_ps(angle2, sign_bit_x);
       angle2= _mm_add_ps(*(const v4sf*)_ps_cephes_PIO2,angle2);
